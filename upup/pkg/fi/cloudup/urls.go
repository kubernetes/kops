...
func BaseUrl(spec *api.ClusterSpec) string {
	...
	if baseUrl == "" {
		...
		kopsUrl := "kops/" + kops.Version + "/"

		if spec.AssetSpec != nil && spec.AssetSpec.FileRepository != nil {
			baseUrl = *spec.AssetSpec.FileRepository + kopsUrl
		} else {
...
func ProtokubeImageSource(spec *api.ClusterSpec) string {
	...
	if protokubeImageSource == "" {
		protokubeImageSource = BaseUrl(spec) + "images/protokube.tar.gz"
