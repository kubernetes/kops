...
func TestGetGroupFailuresStopsOnOldSuccessfulActivity(t *testing.T) {
	asgName := "test-asg"
	asg := &autoscalingtypes.AutoScalingGroup{AutoScalingGroupName: aws.String(asgName)}
	...
	// One recent failure, then nothing but old *successful* activities. Those
	// are filtered out as non-failures, so unless the watermark is checked
	// first they never end the scan and every page gets read.
	activities := []autoscalingtypes.Activity{
		{StartTime: &tAfter, StatusCode: autoscalingtypes.ScalingActivityStatusCodeFailed, StatusMessage: aws.String("err A")},
		{StartTime: &tBefore, StatusCode: autoscalingtypes.ScalingActivityStatusCodeSuccessful, StatusMessage: aws.String("ok 1")},
		{StartTime: &tBefore, StatusCode: autoscalingtypes.ScalingActivityStatusCodeSuccessful, StatusMessage: aws.String("ok 2")},
		{StartTime: &tBefore, StatusCode: autoscalingtypes.ScalingActivityStatusCodeSuccessful, StatusMessage: aws.String("ok 3")},
		{StartTime: &tBefore, StatusCode: autoscalingtypes.ScalingActivityStatusCodeSuccessful, StatusMessage: aws.String("ok 4")},
		{StartTime: &tBefore, StatusCode: autoscalingtypes.ScalingActivityStatusCodeSuccessful, StatusMessage: aws.String("ok 5")},
	}
	...
	if len(got) != 1 || got[0].Message != "err A" {
		t.Errorf("getGroupFailures(%q) = %+v, want just the recent failure", asgName, got)
	}
	// Page 1 is the recent failure, page 2 the first old successful one, which
	// ends the scan. Pages 3-6 must never be requested.
	if calls := asgMock.ScalingActivityCalls(); calls != 2 {
		t.Errorf("getGroupFailures(%q) made %d DescribeScalingActivities calls, want 2", asgName, calls)
	}
}

func TestGetGroupFailuresInProgressActivityDoesNotStopScan(t *testing.T) {
	asgName := "test-asg"
	asg := &autoscalingtypes.AutoScalingGroup{AutoScalingGroupName: aws.String(asgName)}
	...
	// An in-progress activity is listed first and carries the StartTime of when
	// it began, which here predates the watermark. It must not end the scan, or
	// the recent failure behind it would be missed.
	activities := []autoscalingtypes.Activity{
		{StartTime: &tBefore, StatusCode: autoscalingtypes.ScalingActivityStatusCodeInProgress, StatusMessage: aws.String("still launching")},
		{StartTime: &tAfter, StatusCode: autoscalingtypes.ScalingActivityStatusCodeFailed, StatusMessage: aws.String("err A")},
	}
	...
	if len(got) != 1 || got[0].Message != "err A" {
		t.Errorf("getGroupFailures(%q) = %+v, want the failure behind the in-progress activity", asgName, got)
	}
}
