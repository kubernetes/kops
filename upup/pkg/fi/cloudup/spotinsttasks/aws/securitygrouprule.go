...
func (_ *SecurityGroupRule) Render(t *spotinst.Target, a, e, changes *SecurityGroupRule) error {
	...
	if a == nil {
		...
		if fi.BoolValue(e.Egress) {
			...
			_, err := t.Cloud.(spotinst.Cloud).Cloud().(awsup.AWSCloud).EC2().AuthorizeSecurityGroupEgress(request)
			if err != nil {
			...
			_, err := t.Cloud.(spotinst.Cloud).Cloud().(awsup.AWSCloud).EC2().AuthorizeSecurityGroupIngress(request)
			if err != nil {
