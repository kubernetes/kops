...
func (_ *LoadBalancer) modifyLoadBalancerAttributes(t *spotinst.Target, a, e, changes *LoadBalancer) error {
	...
	_, err := t.Cloud.(spotinst.Cloud).Cloud().(awsup.AWSCloud).ELB().ModifyLoadBalancerAttributes(request)
	if err != nil {
