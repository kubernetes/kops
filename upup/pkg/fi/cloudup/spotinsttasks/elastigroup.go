...
func findElastigroup(cloud spotinst.AWSCloud, name string) (*aws.Group, error) {
	output, err := cloud.Elastigroup().List(context.Background(), nil)
	if err != nil {
		return nil, fmt.Errorf("spotinst: failed to find elastigroup %s: %v", name, err)
	}
	if output == nil || (output != nil && len(output.Groups) == 0) {
		return nil, fmt.Errorf("spotinst: failed to find elastigroup %s: got an empty response", name)
	}

	var out *aws.Group
	for _, group := range output.Groups {
		if fi.StringValue(group.Name) == name {
			out = group
			break
		}
	}
	if out == nil {
		return nil, fmt.Errorf("spotinst: failed to find elastigroup %s: elastigroup does not exist", name)
	}
...
func (e *Elastigroup) Find(c *fi.Context) (*Elastigroup, error) {
	cloud := c.Cloud.(spotinst.AWSCloud)

	group, err := findElastigroup(cloud, *e.Name)
	if err != nil {
		return nil, err
	}
	if group == nil {
	...
	// Launch Specification.
	{
		...
		// Image.
		{
			image, err := cloud.ResolveImage(*lc.ImageID)
			if err != nil {
...
func (_ *Elastigroup) RenderAWS(t *spotinst.AWSAPITarget, a, e, changes *Elastigroup) error {
	cloud := t.Cloud.(spotinst.AWSCloud)
	...
	if a == nil {
		...
		// Strategy.
		{
			...
			group.Strategy.SetFallbackToOnDemand(fi.Bool(true))
			group.Strategy.SetUtilizeReservedInstances(fi.Bool(true))
		}
	...
	readyLoop:
		for {
			...
			out, err := cloud.Elastigroup().Create(context.Background(), input)
			if err == nil {
				e.ID = out.Group.ID
				break
			}
		...
		actual, err := findElastigroup(cloud, *e.Name)
		if err != nil {
		...
		if !reflect.DeepEqual(empty, changes) {
			glog.Warningf("Not all changes applied to elastigroup: %v", changes)
		}
...
func (_ *Elastigroup) RenderTerraform(t *terraform.TerraformTarget, a, e, changes *Elastigroup) error {
	...
	tf := &terraformElastigroup{
		...
		terraformElastigroupStrategy: &terraformElastigroupStrategy{
			...
			FallbackToOnDemand:       fi.Bool(true),
			UtilizeReservedInstances: fi.Bool(true),
		},
	...
	// Block Devices.
	{
		rootDevices, err := e.buildRootDevice(t.Cloud.(spotinst.AWSCloud))
		if err != nil {
			return err
		}
	...
	// Integration.
	{
		if e.IntegrationClusterIdentifier != nil {
			tf.Integration = &terraformElastigroupIntegration{
				...
				ClusterIdentifier: e.IntegrationClusterIdentifier,
			}
			if e.IntegrationAutoScaleEnabled != nil {
				tf.Integration.AutoScaleIsEnabled = e.IntegrationAutoScaleEnabled
			}
		}
	}
...
func (e *Elastigroup) buildRootDevice(cloud spotinst.AWSCloud) (map[string]*awstasks.BlockDeviceMapping, error) {
...
func (e *Elastigroup) applyDefaults() {
	if v := e.Product; v == nil || (v != nil && fi.StringValue(v) == "") {
		e.Product = fi.String("Linux/UNIX")
	}
	if v := e.Orientation; v == nil || (v != nil && fi.StringValue(v) == "") {
