...
func (c *ApplyClusterCmd) Run() error {
	...
	if c.NodeUpSource == "" {
		...
		if location == "" {
			location = kops.DefaultNodeUpLocation()
	...
	renderNodeUpConfig := func(ig *api.InstanceGroup) (*nodeup.NodeUpConfig, error) {
		...
		{
			location := os.Getenv("PROTOKUBE_IMAGE")
			if location == "" {
				location = kops.DefaultProtokubeImageSource()
				glog.V(2).Infof("Using default protokube location: %q", location)
			} else {
				glog.Warningf("Using protokube location from PROTOKUBE_IMAGE env var: %q", location)
			}
