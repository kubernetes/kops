...
func TestRemapFileToOCI(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			info:       FileAssetInfo{Family: "containerd", Version: "v2.2.4", Architecture: "amd64"},
			want:       "oci://index.docker.io/example/containerd:v2.2.4-amd64",
		},
		{
			name:       "Docker Hub registry endpoint",
			repository: "oci://index.docker.io/example",
			info:       FileAssetInfo{Family: "containerd", Version: "v2.2.4", Architecture: "amd64"},
			want:       "oci://index.docker.io/example/containerd:v2.2.4-amd64",
		},
		{
		...
		t.Run(test.name, func(t *testing.T) {
			builder := NewAssetBuilder(nil, &kops.AssetsSpec{FileRepository: &test.repository}, true)
...
func TestRemapFileToOCIUsesExactSourceSHA256(t *testing.T) {
	...
	builder := NewAssetBuilder(vfs.Context, &kops.AssetsSpec{FileRepository: &repository}, true)
