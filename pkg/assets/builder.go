...
func (a *AssetBuilder) RemapFileWithInfo(canonicalURL *url.URL, knownHash *hashing.Hash, info FileAssetInfo) (*FileAsset, error) {
	...
	if isOCI && !a.getAssets && knownHash == nil {
		// Local pins remain authoritative without requiring access to the source.
		var err error
		knownHash, _, err = assetdata.GetHash(canonicalURL)
		if err != nil {
			return nil, err
		}
	}
	if isOCI && a.getAssets && knownHash == nil {
		// Staging needs the source hash to verify the bytes and name hash-derived tags.
		var err error
	...
	if isOCI && knownHash != nil && knownHash.Algorithm != hashing.HashAlgorithmSHA256 {
		if !a.getAssets {
			return nil, fmt.Errorf("OCI asset %q requires a SHA-256 for updates, found %s", canonicalURL, knownHash.Algorithm)
		}
		var err error
	...
	if isOCI && !a.getAssets && knownHash == nil {
		// Unpinned updates resolve the staged file layer without accessing the source.
		h, err := findOCIHash(fileAsset.DownloadURL)
		if err != nil {
			return nil, err
		}
		knownHash = h
	} else if knownHash == nil {
...
func (a *AssetBuilder) remapURL(canonicalURL *url.URL, info FileAssetInfo, hash *hashing.Hash) (*url.URL, error) {
	...
	if fileRepo.Scheme == "oci" {
		// Match go-containerregistry's Docker Hub alias so bootstrap curl uses the registry endpoint.
		if fileRepo.Host == "docker.io" {
			fileRepo.Host = "index.docker.io"
		}
		family := info.Family
		tag, err := ociAssetTag(info, hash)
		if err != nil {
			return nil, fmt.Errorf("naming OCI asset %q: %w", canonicalURL, err)
		}
...
func ociAssetTag(info FileAssetInfo, hash *hashing.Hash) (string, error) {
	...
	if hash == nil || hash.Algorithm != hashing.HashAlgorithmSHA256 {
		return "", fmt.Errorf("a version that fits in an OCI tag or an explicit SHA-256 is required; updates do not fetch upstream checksums")
	}
	tag := "sha256-" + hash.Hex()
	if info.Architecture != "" {
		tag += "-" + info.Architecture
