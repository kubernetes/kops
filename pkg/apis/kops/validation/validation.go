...
func validateFileAssetSpec(v *kops.FileAssetSpec, fieldPath *field.Path) field.ErrorList {
	...
	// nodeup writes an asset without a path below the file assets directory.
	assetPaths := []string{v.Path}
	if v.Path == "" {
		assetPaths = nil
		for _, srvKubernetes := range srvKubernetesDirs {
			assetPaths = append(assetPaths, filepath.Join(srvKubernetes, "assets", v.Name))
		}
	}
	for _, assetPath := range assetPaths {
		if isReservedKubeAPIServerPath(assetPath) {
			allErrs = append(allErrs, field.Forbidden(fieldPath.Child("path"), reservedKubeAPIServerPathMessage))
			break
		}
	}
...
// srvKubernetesDirs are the locations of the kubernetes config directory on the nodes.
var srvKubernetesDirs = []string{"/srv/kubernetes", "/etc/srv/kubernetes"}

const reservedKubeAPIServerPathMessage = "this path is reserved for the authentication configuration generated by kOps"

// isReservedKubeAPIServerPath returns true if p is the kube-apiserver AuthenticationConfiguration
// file that nodeup generates, in either location of the kubernetes config directory.
func isReservedKubeAPIServerPath(p string) bool {
	if p == "" {
		return false
	}
	for _, srvKubernetes := range srvKubernetesDirs {
		if filepath.Clean(p) == filepath.Join(srvKubernetes, "kube-apiserver", wellknownpaths.KubeAPIServerAuthenticationConfigFileName) {
			return true
		}
	}
	return false
...
func validateKubeAPIServer(v *kops.KubeAPIServerConfig, c *kops.Cluster, fldPath *field.Path, strict bool) field.ErrorList {
	...
	if isReservedKubeAPIServerPath(v.AuthenticationConfigFile) {
		allErrs = append(allErrs, field.Forbidden(fldPath.Child("authenticationConfigFile"), reservedKubeAPIServerPathMessage))
	}
	if isReservedKubeAPIServerPath(fi.ValueOf(v.OIDCCAFile)) {
		allErrs = append(allErrs, field.Forbidden(fldPath.Child("oidcCAFile"), reservedKubeAPIServerPathMessage))
	}
