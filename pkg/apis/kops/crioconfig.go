...
type CrioConfig struct {
	...
	// TODO: since now that we are using a TOML based file generator, we can afford to specify extra options to configure cri-o without much effort
	ConfigOverride *string `json:"configOverride,omitempty"`
	// ContainerPolicyOverride is the image signature verification policy provided by the user. It goes to /etc/containers/policy.json
	...
	// Version determines the version of crio package to install.
