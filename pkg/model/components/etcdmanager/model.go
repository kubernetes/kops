...
// buildPod creates the pod spec, based on the etcd cluster
func (b *EtcdManagerBuilder) buildPod(etcdCluster *kops.EtcdClusterSpec) (*v1.Pod, error) {
	var pod *v1.Pod
	var container *v1.Container

	bundle := etcdCluster.Manager.Bundle
	var manifest []byte

	if bundle == "" {
		bundle = "(embedded bundle)"

		// TODO: Load static bundle?
		return nil, fmt.Errorf("TODO: what if not bundle")

		// TODO: How to turn this off?
	} else {
		yamlBytes, err := bundles.LoadBundleManifest(&b.Cluster.Spec, bundle)
		if err != nil {
			return nil, err
		}
