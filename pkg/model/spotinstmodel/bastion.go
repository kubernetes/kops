...
func (b *BastionModelBuilder) Build(c *fi.ModelBuilderContext) error {
	...
	// TODO: Could we get away without an ELB here?  Tricky to fix if dns-controller breaks though...
	{
		t := &awstasks.SecurityGroupRule{
			...
			SecurityGroup: b.LinkToSecurityGroup(kops.InstanceGroupRoleBastion),
			SourceGroup:   b.LinkToELBSecurityGroup(model.BastionELBSecurityGroupPrefix),
	...
	{
		t := &awstasks.SecurityGroup{
			Name:      fi.String(b.ELBSecurityGroupName(model.BastionELBSecurityGroupPrefix)),
			Lifecycle: b.SecurityLifecycle,
	...
	// Allow traffic from ELB to egress freely
	{
		t := &awstasks.SecurityGroupRule{
			...
			SecurityGroup: b.LinkToELBSecurityGroup(model.BastionELBSecurityGroupPrefix),
			Egress:        fi.Bool(true),
			CIDR:          fi.String("0.0.0.0/0"),
		}
	...
	var elb *awstasks.LoadBalancer
	{
		...
		idleTimeout := model.BastionELBDefaultIdleTimeout
		...
		elb = &awstasks.LoadBalancer{
			...
			SecurityGroups: []*awstasks.SecurityGroup{
				b.LinkToELBSecurityGroup(model.BastionELBSecurityGroupPrefix),
			},
			Subnets: elbSubnets,
