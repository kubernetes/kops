...
func TestNodeUpBootstrapDockerHub(t *testing.T) {
	repository := "oci://docker.io/example"
	builder := assets.NewAssetBuilder(nil, &kops.AssetsSpec{FileRepository: &repository}, true)
	hash := hashing.MustFromString("833723369ad345a88dd85d61b1e77336d56e61b864557ded71b92b6e34158e6a")
	asset, err := builder.RemapFileWithInfo(
		&url.URL{Scheme: "https", Host: "artifacts.example.com", Path: "/nodeup.xz"},
		hash,
		assets.FileAssetInfo{Family: "nodeup", Version: "1.37.0", Architecture: "amd64"},
	)
	if err != nil {
		t.Fatal(err)
	}
	rendered := renderNodeUpScript(t, &NodeUpScript{
		NodeUpAssets: map[architectures.Architecture]*assets.MirroredAsset{
			architectures.ArchitectureAmd64: assets.BuildMirroredAsset(asset),
		},
	})
	want := "NODEUP_URL_AMD64=oci://index.docker.io/example/nodeup:v1.37.0-amd64\n"
	if !strings.Contains(rendered, want) {
		t.Fatalf("bootstrap script does not contain %q", want)
	}
}
