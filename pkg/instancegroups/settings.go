...
func resolveSettings(cluster *kops.Cluster, group *kops.InstanceGroup, numInstances int) kops.RollingUpdate {
	...
	if rollingUpdate.MaxSurge == nil {
		...
		if kops.CloudProviderID(cluster.Spec.CloudProvider) == kops.CloudProviderAWS && !featureflag.Spotinst.Enabled() && group.Spec.InstanceManager != kops.InstanceManagerKarpenter {
