...
func RunToolboxInventory(f *util.Factory, out io.Writer, options *GetInventoryOptions) error {
	...
	case OutputJSON:
		...
		if err != nil {
			return fmt.Errorf("unable to marshal inventory as json: %v", err)
		}

		if _, err := fmt.Fprintf(out, string(j)); err != nil {
			return fmt.Errorf("unable to output json: %v", err)
		}

	case OutputYaml:
		y, err := yaml.Marshal(a)
		if err != nil {
			return fmt.Errorf("unable to marshal inventory as yaml: %v", err)
		}

		if _, err := fmt.Fprintf(out, string(y)); err != nil {
			return fmt.Errorf("unable to output json: %v", err)
		}
	default:
