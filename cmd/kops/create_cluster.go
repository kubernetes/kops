...
func NewCmdCreateCluster(f *util.Factory, out io.Writer) *cobra.Command {
	...
	cmd.Flags().StringVar(&options.MasterVolumeSize, "master-volume-size", options.MasterVolumeSize, "Set instance size for masters")
	cmd.Flags().StringVar(&options.NodeVolumeSize, "node-volume-size", options.NodeVolumeSize, "Set instance volume size for nodes")
