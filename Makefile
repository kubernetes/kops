...
channels:
	...
	GOOS=linux GOARCH=amd64 go build ${GCFLAGS} ${BUILDFLAGS} ${EXTRA_BUILDFLAGS} -o ${DIST}/linux/amd64/channels ${LDFLAGS}"${EXTRA_LDFLAGS} -X k8s.io/kops.Version=${VERSION}" k8s.io/kops/channels/cmd/channels
...
gcs-upload-and-tag: gsutil gcs-upload
	...
	gcloud storage cp ${UPLOAD}/${LATEST_FILE} ${GCS_BUCKET}/${LATEST_FILE} \
	  --custom-metadata="Surrogate-Key=kops-ci-markers"
	curl -fsSL -o /tmp/fastly.tar.gz https://github.com/fastly/cli/releases/download/v15.1.0/fastly_v15.1.0_linux-amd64.tar.gz
	tar -xzf /tmp/fastly.tar.gz -C /tmp
	/tmp/fastly purge --service-name=${FASTLY_SERVICE_NAME} \
  	  --key kops-ci-markers --non-interactive
...
ko-kops-channels-push:
	KO_DOCKER_REPO="${DOCKER_IMAGE_REGISTRY}/channels" ${KO} build --tags ${KOPS_CHANNELS_TAG} --platform=linux/amd64,linux/arm64 --bare ./channels/cmd/channels/
