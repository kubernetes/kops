...
When `kubernetesVersion` is a URL, kOps uses its final version directory, such as
`v1.37.0-alpha.0.123+abcdef`, for both binary and component image tags. Restage these assets if
they were previously copied under hash-derived tags.

During cluster updates, kOps preserves an explicitly configured SHA-256, or otherwise uses its
embedded well-known hash. Only assets without either pin use the staged manifest's
`layers[0].digest`. Updates do not fetch upstream checksum files or source files. Assets without a
version must have an explicit or embedded SHA-256 to identify their hash-derived tag. Explicit
hashes using other algorithms must be replaced with SHA-256 for OCI updates. Staging still
resolves and verifies the source bytes before publishing them.
