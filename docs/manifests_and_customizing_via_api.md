...
Using a document based configuration for building and managing `kops` clusters is powerful.  Do use these features with caution.  If you do not need to define or customize a value, let `kops` create and possibly modify the value by default.  Again do not use the full ClusterSpec.  Users have wasted days debugging issues, due to using the full specification.  But if you need to run a custom version of Kubernetes Controller Manager, set `kubeControllerManager.image` and update your cluster.  Do take care with setting custom values and test before using in production.
