...
```
      ...
      --bastion-loadbalancer-type string   Sets the Bastion loadbalancer type to either 'public' or 'internal'
  ...
  -o, --output string                      Output format. One of json|yaml. Used with the --dry-run flag.
      ...
      --ssh-allow-list strings             Restrict SSH access to this CIDR.  If not set, access will not be restricted by IP. (default [0.0.0.0/0])
