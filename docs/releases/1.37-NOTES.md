...
As part of this removal, nodes no longer install or run the `protokube` component, whose only remaining responsibility was gossip DNS. The `gossipConfig` and `dnsControllerGossipConfig` cluster spec fields are deprecated, no longer have any effect, and must be removed from cluster specs.
...
* Nodes no longer install or run `protokube`.
