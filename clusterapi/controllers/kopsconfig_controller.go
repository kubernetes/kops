...
func (r *KopsConfigReconciler) buildBootstrapData(ctx context.Context, cluster *kopsapi.Cluster, kopsControlPlane *v1beta1.KopsControlPlane) ([]byte, error) {
	...
	// clusterStoreBasePath := "gs://kops-state-store"
...
func BuildNodeupConfigFromScratch(ctx context.Context, kube client.Client, clusterName string) (*NodeupConfig, error) {
	cluster := &v1alpha2.Cluster{}
	// TODO: Use cached client
	id := types.NamespacedName{
		Namespace: "kube-system",
		Name:      clusterName,
	}
	if err := kube.Get(ctx, id, cluster); err != nil {
		return nil, fmt.Errorf("getting cluster %v: %w", id, err)
	}

	capiCluster, err := getCAPIClusterFromKopsCluster(ctx, kube, cluster)
	if err != nil {
		return nil, err
	}

	kopsControlPlane, err := getKopsControlPlaneFromCAPICluster(ctx, kube, capiCluster)
	if err != nil {
		return nil, err
	}

	return BuildNodeupConfig(ctx, cluster, kopsControlPlane)
}

// TODO: Dedup with b.builder.NodeUpConfigBuilder.BuildConfig
func BuildNodeupConfig(ctx context.Context, cluster *kopsapi.Cluster, kopsControlPlane *v1beta1.KopsControlPlane) (*NodeupConfig, error) {
	...
	for _, systemEndpoint := range kopsControlPlane.Status.SystemEndpoints {
		...
		case "kops-controller":
			wellKnownAddresses[wellknownservices.KopsController] = append(wellKnownAddresses[wellknownservices.KopsController], systemEndpoint.Host)
		case "kube-apiserver":
			wellKnownAddresses[wellknownservices.KubeAPIServer] = append(wellKnownAddresses[wellknownservices.KubeAPIServer], systemEndpoint.Host)
		}
	}
