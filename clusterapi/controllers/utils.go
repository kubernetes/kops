...
func getCAPIClusterFromKopsCluster(ctx context.Context, kube client.Client, kopsCluster *v1alpha2.Cluster) (*unstructured.Unstructured, error) {
	var capiClusters unstructured.UnstructuredList
	capiClusters.SetGroupVersionKind(schema.GroupVersionKind{
		Group:   "cluster.x-k8s.io",
		Version: "v1beta1",
		Kind:    "Cluster",
	})
	if err := kube.List(ctx, &capiClusters, client.InNamespace(kopsCluster.GetNamespace())); err != nil {
		return nil, fmt.Errorf("error listing clusters in namespace %v: %w", kopsCluster.GetNamespace(), err)
	}

	var matches []*unstructured.Unstructured

	for i := range capiClusters.Items {
		capiCluster := &capiClusters.Items[i]
		for _, ownerRef := range capiCluster.GetOwnerReferences() {
			if ownerRef.Kind == "Cluster" && strings.HasPrefix(ownerRef.APIVersion, "kops.k8s.io/") {
				if ownerRef.Name == kopsCluster.GetName() {
					matches = append(matches, capiCluster)
				}
			}
		}
	}
	...
	if len(matches) == 0 {
		return nil, fmt.Errorf("no matching CAPI cluster found")
	}
	if len(matches) > 1 {
		return nil, fmt.Errorf("multiple matching CAPI clusters found")
	}
