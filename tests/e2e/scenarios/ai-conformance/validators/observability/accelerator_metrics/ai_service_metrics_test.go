...
func TestObservability_AIServiceMetrics_ViaPrometheus(t *testing.T) {
	...
	h.Run("ai-service-metrics", func(h *validators.ValidatorHarness) {
		...
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			if err := json.Unmarshal([]byte(output), &promResponse); err != nil {
				h.Errorf("Failed to parse Prometheus response as JSON: %v", err)
			} else if promResponse.Status != "success" {
				h.Errorf("Prometheus query did not return success status: %s", promResponse.Status)
			} else {
				for _, result := range promResponse.Data.Result {
					if result.Metric["namespace"] == ns && result.Metric["__name__"] == metricName {
						found = true
						h.Logf("Prometheus collected metric %s on attempt %d: %+v", metricName, attempt, result)
					}
				}
			}
