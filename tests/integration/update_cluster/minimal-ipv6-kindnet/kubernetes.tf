...
resource "aws_s3_object" "minimal-ipv6-example-com-addons-networking-kindnet-k8s-1-32" {
  bucket                 = "testingBucket"
  content                = file("${path.module}/data/aws_s3_object_minimal-ipv6.example.com-addons-networking.kindnet-k8s-1.32_content")
  key                    = "clusters.example.com/minimal-ipv6.example.com/addons/networking.kindnet/k8s-1.32.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
