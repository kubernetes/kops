...
resource "aws_autoscaling_group" "master-us-test-1a-masters-minimal-ipv6-example-com" {
  ...
  target_group_arns   = [aws_lb_target_group.kops-controller-minimal-i-fg70er.id, aws_lb_target_group.tcp-minimal-ipv6-example--bne5ih.id]
...
resource "aws_lb_listener" "api-minimal-ipv6-example-com-3988" {
  default_action {
    target_group_arn = aws_lb_target_group.kops-controller-minimal-i-fg70er.id
    type             = "forward"
  }
...
resource "aws_lb_target_group" "kops-controller-minimal-i-fg70er" {
  ...
  name     = "kops-controller-minimal-i-fg70er"
  port     = 3988
  protocol = "TCP"
  tags = {
    "KubernetesCluster"                              = "minimal-ipv6.example.com"
