...
output "iam_openid_connect_provider_arn" {
  value = aws_iam_openid_connect_provider.minimal-k8s-local.arn
}
...
resource "aws_iam_openid_connect_provider" "minimal-k8s-local" {
...
resource "aws_s3_object" "manifests-channels-kops-channels" {
  ...
  key                    = "clusters.example.com/minimal.k8s.local/manifests/channels/kops-channels.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
...
resource "aws_s3_object" "nodeupconfig-master-us-test-1a" {
  ...
  key                    = "clusters.example.com/minimal.k8s.local/igconfig/control-plane/master-us-test-1a/nodeupconfig.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
