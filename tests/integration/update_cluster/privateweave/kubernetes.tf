...
resource "aws_autoscaling_group" "bastion-privateweave-example-com" {
  ...
  target_group_arns   = [aws_lb_target_group.bastion-privateweave-exam-fdb6ge.id]
...
resource "aws_lb" "bastion-privateweave-example-com" {
...
resource "aws_lb_listener" "bastion-privateweave-example-com-22" {
  default_action {
    target_group_arn = aws_lb_target_group.bastion-privateweave-exam-fdb6ge.id
    type             = "forward"
  }
  load_balancer_arn = aws_lb.bastion-privateweave-example-com.id
  port              = 22
  protocol          = "TCP"
}
...
resource "aws_s3_object" "manifests-etcdmanager-events-master-us-test-1a" {
  ...
  key                    = "clusters.example.com/privateweave.example.com/manifests/etcd/events-master-us-test-1a.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
...
resource "aws_s3_object" "manifests-etcdmanager-main-master-us-test-1a" {
  ...
  key                    = "clusters.example.com/privateweave.example.com/manifests/etcd/main-master-us-test-1a.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
...
resource "aws_s3_object" "nodeupconfig-master-us-test-1a" {
  ...
  key                    = "clusters.example.com/privateweave.example.com/igconfig/control-plane/master-us-test-1a/nodeupconfig.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
...
resource "aws_s3_object" "privateweave-example-com-addons-aws-ebs-csi-driver-addons-k8s-io-k8s-1-17" {
  bucket                 = "testingBucket"
  content                = file("${path.module}/data/aws_s3_object_privateweave.example.com-addons-aws-ebs-csi-driver.addons.k8s.io-k8s-1.17_content")
  key                    = "clusters.example.com/privateweave.example.com/addons/aws-ebs-csi-driver.addons.k8s.io/k8s-1.17.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
...
resource "aws_security_group_rule" "from-0-0-0-0--0-ingress-tcp-22to22-bastion-privateweave-example-com" {
...
resource "aws_security_group_rule" "from-172-20-4-0--22-ingress-tcp-22to22-bastion-privateweave-example-com" {
