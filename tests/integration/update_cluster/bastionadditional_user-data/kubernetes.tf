...
resource "aws_autoscaling_group" "bastion-bastionuserdata-example-com" {
  ...
  target_group_arns   = [aws_lb_target_group.bastion-bastionuserdata-e-4grhsv.id]
...
resource "aws_lb_listener" "bastion-bastionuserdata-example-com-22" {
  default_action {
    target_group_arn = aws_lb_target_group.bastion-bastionuserdata-e-4grhsv.id
    type             = "forward"
  }
  load_balancer_arn = aws_lb.bastion-bastionuserdata-example-com.id
  port              = 22
  protocol          = "TCP"
}
