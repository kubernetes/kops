...
resource "aws_s3_object" "manifests-channels-kops-channels" {
  ...
  key                    = "tests/minimal-gce-private.example.com/manifests/channels/kops-channels.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
...
resource "google_compute_instance_group_manager" "b-nodes-minimal-gce-private-example-com" {
  base_instance_name = "nodes"
  lifecycle {
    ignore_changes = [target_size]
  }
