...
resource "aws_autoscaling_group" "bastion-privateciliumadvanced-example-com" {
  ...
  target_group_arns   = [aws_lb_target_group.bastion-privateciliumadva-0jni40.id]
...
resource "aws_lb_listener" "bastion-privateciliumadvanced-example-com-22" {
  default_action {
    target_group_arn = aws_lb_target_group.bastion-privateciliumadva-0jni40.id
    type             = "forward"
  }
  load_balancer_arn = aws_lb.bastion-privateciliumadvanced-example-com.id
  port              = 22
  protocol          = "TCP"
}
...
resource "aws_s3_object" "nodeupconfig-master-us-test-1a" {
  ...
  key                    = "clusters.example.com/privateciliumadvanced.example.com/igconfig/control-plane/master-us-test-1a/nodeupconfig.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
...
resource "aws_s3_object" "privateciliumadvanced-example-com-addons-aws-cloud-controller-addons-k8s-io-k8s-1-18" {
  bucket                 = "testingBucket"
  content                = file("${path.module}/data/aws_s3_object_privateciliumadvanced.example.com-addons-aws-cloud-controller.addons.k8s.io-k8s-1.18_content")
  key                    = "clusters.example.com/privateciliumadvanced.example.com/addons/aws-cloud-controller.addons.k8s.io/k8s-1.18.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}

resource "aws_s3_object" "privateciliumadvanced-example-com-addons-aws-ebs-csi-driver-addons-k8s-io-k8s-1-17" {
  bucket                 = "testingBucket"
  content                = file("${path.module}/data/aws_s3_object_privateciliumadvanced.example.com-addons-aws-ebs-csi-driver.addons.k8s.io-k8s-1.17_content")
  key                    = "clusters.example.com/privateciliumadvanced.example.com/addons/aws-ebs-csi-driver.addons.k8s.io/k8s-1.17.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
