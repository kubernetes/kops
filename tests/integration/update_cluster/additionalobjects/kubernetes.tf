...
output "vpc_ipv6_cidr_block" {
  value = aws_vpc.additionalobjects-example-com.ipv6_cidr_block
}
...
resource "aws_s3_object" "additionalobjects-example-com-addons-aws-cloud-controller-addons-k8s-io-k8s-1-18" {
  bucket                 = "testingBucket"
  content                = file("${path.module}/data/aws_s3_object_additionalobjects.example.com-addons-aws-cloud-controller.addons.k8s.io-k8s-1.18_content")
  key                    = "tests/additionalobjects.example.com/addons/aws-cloud-controller.addons.k8s.io/k8s-1.18.yaml"
  provider               = aws.files
...
resource "aws_s3_object" "manifests-channels-kops-channels" {
  ...
  key                    = "tests/additionalobjects.example.com/manifests/channels/kops-channels.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
...
resource "aws_s3_object" "nodeupconfig-master-us-test-1a" {
  ...
  key                    = "tests/additionalobjects.example.com/igconfig/control-plane/master-us-test-1a/nodeupconfig.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
