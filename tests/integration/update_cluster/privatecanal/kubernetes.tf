...
resource "aws_autoscaling_group" "bastion-privatecanal-example-com" {
  ...
  target_group_arns   = [aws_lb_target_group.bastion-privatecanal-exam-hmhsp5.id]
...
resource "aws_lb" "bastion-privatecanal-example-com" {
...
resource "aws_lb_listener" "bastion-privatecanal-example-com-22" {
  default_action {
    target_group_arn = aws_lb_target_group.bastion-privatecanal-exam-hmhsp5.id
    type             = "forward"
  }
  load_balancer_arn = aws_lb.bastion-privatecanal-example-com.id
  port              = 22
  protocol          = "TCP"
}
...
resource "aws_s3_object" "nodeupconfig-master-us-test-1a" {
  ...
  key                    = "clusters.example.com/privatecanal.example.com/igconfig/control-plane/master-us-test-1a/nodeupconfig.yaml"
  provider               = aws.files
  server_side_encryption = "AES256"
}
