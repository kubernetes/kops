...
function generate_cni_hashes_legacy() {
  prefix=$1
  patch=$2

  cat > "${REPO_ROOT}/pkg/assets/assetdata/cni-legacy-${prefix}.yaml" <<EOF
# This file is generated by generate-asset-hashes.sh
...
EOF
...

  {
    ...
    go run ./pkg/assets/assetdata/tools/cmd/generatefileassets \
      ...
      | grep -v "^files:"
  } >> "${REPO_ROOT}/pkg/assets/assetdata/cni-legacy-${prefix}.yaml"
}
...
EOF
  ...
  # Iterate over each version provided in the argument list
  for version in "${versions[@]}"; do
    ...
    go run ./pkg/assets/assetdata/tools/cmd/generatefileassets \
      ...
      --prefix "v${version}/" \
      --sums "https://github.com/containernetworking/plugins/releases/download/v${version}/cni-plugins-linux-amd64-v${version}.tgz.sha256" \
      | grep -v "^files:"
    go run ./pkg/assets/assetdata/tools/cmd/generatefileassets \
      --base https://github.com/containernetworking/plugins/releases/download/ \
      --prefix "v${version}/" \
      --sums "https://github.com/containernetworking/plugins/releases/download/v${version}/cni-plugins-linux-arm64-v${version}.tgz.sha256" \
      | grep -v "^files:"
...
generate_cni_hashes_legacy 0.9 1
generate_cni_hashes_legacy 1.2 0
generate_cni_hashes_legacy 1.3 0
generate_cni_hashes_legacy 1.4 1
generate_cni_hashes_legacy 1.5 1
generate_cni_hashes_legacy 1.6 1
# Generate CNI hashes, from github.com/containernetworking/plugins/releases/download/
# Note: argument is now base path and list of versions (because there is no 1.6.1)
generate_cni_hashes 1.6 1.6.0 1.6.2
