...
type AccessReviewScheduleDefinitionsAssignedForMyApprovalClientListOptions struct {
	// The filter to apply on the operation. One custom filter option is supported : 'assignedToMeToReview()'. When specified
	// $filter=assignedToMeToReview(), only items that are assigned to the calling user to review are returned
	Filter *string
}
...
type DenyAssignmentsClientListForResourceGroupOptions struct {
	...
	// eq '{name}' to search deny assignments by name at specified scope. Use $filter=principalId eq '{id}' to return all deny
	// assignments at, above and below the scope for the specified principal. Use $filter=gdprExportPrincipalId eq '{id}' to return
	// all deny assignments at, above and below the scope for the specified principal. This filter is different from the principalId
	// filter as it returns not only those deny assignments that contain the specified principal is the Principals list but also
	// those deny assignments that contain the specified principal is the ExcludePrincipals list. Additionally, when gdprExportPrincipalId
...
type EligibleChildResourcesClientGetOptions struct {
	...
	// Use $filter=resourceType+eq+'subscription'+or+resourceType+eq+'resourcegroup' to filter on resource of type = 'Subscription'
	// or 'ResourceGroup'
	Filter *string
}
