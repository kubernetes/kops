...
// CheckExistenceByID checks by ID whether a resource exists. This API currently works only for a limited set of
// Resource providers. In the event that a Resource provider does not implement this API, ARM will respond with a 405.
// The alternative then is to use the GET API to check for the existence of the resource.
// Parameters:
...
// Parameters:
// filter - the filter to apply on the operation.<br><br>Filter comparison operators include `eq` (equals) and
// `ne` (not equals) and may be used with the following properties: `location`, `resourceType`, `name`,
...
// `$filter=resourceType eq 'Microsoft.Network/virtualNetworks'`<br><br><br>`substringof(value, property)` can
// be used to filter for substrings of the following currently-supported properties: `name` and
// `resourceGroup`<br><br>For example, to get all resources with 'demo' anywhere in the resource name, use
// `$filter=substringof('demo', name)`<br><br>Multiple substring operations can also be combined using
// `and`/`or` operators.<br><br>Note that any truncated number of results queried via `$top` may also not be
// compatible when using a filter.<br><br><br>Resources can be filtered by tag names and values. For example,
// to filter for a tag name and value, use `$filter=tagName eq 'tag1' and tagValue eq 'Value1'`. Note that when
// resources are filtered by tag name and value, <b>the original tags for each resource will not be returned in
// the results.</b> Any list of additional properties queried via `$expand` may also not be compatible when
// filtering by tag names/values. <br><br>For tag names only, resources can be filtered by prefix using the
// following syntax: `$filter=startswith(tagName, 'depart')`. This query will return all resources with a tag
// name prefixed by the phrase `depart` (i.e.`department`, `departureDate`, `departureTime`,
// etc.)<br><br><br>Note that some properties can be combined when filtering resources, which include the
// following: `substringof() and/or resourceType`, `plan and plan/publisher and plan/name`, and `identity and
...
// MoveResources the resources to be moved must be in the same source resource group in the source subscription being
// used. The target resource group may be in a different subscription. When moving resources, both the source group and
// the target group are locked for the duration of the operation. Write and delete operations are blocked on the groups
// until the move completes.
// Parameters:
// sourceResourceGroupName - the name of the resource group from the source subscription containing the
...
// resources to be moved must be in the same source resource group in the source subscription being used. The target
// resource group may be in a different subscription. If validation succeeds, it returns HTTP response code 204 (no
// content). If validation fails, it returns HTTP response code 409 (Conflict) with an error message. Retrieve the URL
// in the Location header value to check the result of the long-running operation.
// Parameters:
// sourceResourceGroupName - the name of the resource group from the source subscription containing the
